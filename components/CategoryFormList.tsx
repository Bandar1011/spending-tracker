...
export function CategoryFormList() {
  ...
  const totals = useMemo(() => ({ allocated: undefined, unallocated: undefined, over: false }), []);
  ...
  function addRow() {
    ...
    setRows((prev) => [...prev, { id, name: "", isArchived: false } as Category]);
  }
        ...
        <div className="grid grid-cols-1 gap-2">
          {rows.map((r) => (
            <div key={r.id} className="grid grid-cols-12 gap-2 items-center">
              <Input className="col-span-8" value={r.name} onChange={(e) => updateRow(r.id, { name: e.target.value })} placeholder="Category name" />
              <div className="col-span-2 text-sm text-muted-foreground">{r.isArchived ? "Archived" : "Active"}</div>
